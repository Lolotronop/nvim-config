...
local function qf_jump(cmd, wrap)
    local n = vim.fn.getqflist({ size = 1 }).size
    if n == 0 then return end
    if not pcall(vim.cmd, n == 1 and "cc" or cmd) then vim.cmd(wrap) end
end

vim.keymap.set("n", "<M-l>", function() qf_jump("cnext", "cfirst") end, { desc = "Next [L]ist" })
vim.keymap.set("n", "<M-h>", function() qf_jump("cprevious", "clast") end, { desc = "Previous [L]ist" })
